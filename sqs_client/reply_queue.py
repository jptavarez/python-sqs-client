...
class ReplyQueue(ReplyQueueBase):
    ...
    def __init__(self,
        ...
        self._messages = {}
        self._run_receive_messages_thread = True

    def stop_receive_messages_thread(self):
        self._run_receive_messages_thread = False
        self._sub_thread.join()
        return None

    def get_url(self):
        # this creates the temp SQS Queue and attaches a listener (Subscriber)
        # in a subthread.  The lisetener quests messages and puts them in a list.
        if not self._queue:
            self._build_queue()
    ...
    def _receive_messages(self):
        ...
        while True:
            qty_messages = 0
            # below is a subscriber that has a generator (really a co-routine) which listens to a queue and pushes messages (long polling)
            # in batches to create lists of messages the messages are pushed into a self._messages[message.request_id]
            # and deleted from the messages list.
            for messages in self._subscriber.receive_messages(message_attribute_names=['RequestMessageId']):
                ...
                # delete the messages b/c they have been added to class variable
                # next repeat of the loop will get more from the co-routine which
                # is pushing into this loop
                messages.delete()
                if qty_messages >= self._num_messages_before_cleaning:
                    break
            self._clean_old_messages()
            if self._run_receive_messages_thread is False:
                break
            # just keep processing return messages and staging them into self._messages
